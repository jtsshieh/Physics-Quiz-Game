...
export function HorizontalArrow({ x1, x2, y }: {x1: number, x2: number, y: number}) {
    return <line
        x1={x1}
        y1={y}
        x2={x2}
...
export function VerticalArrow({ y1, y2, x }: { y1: number, y2: number, x: number }) {
    return <line
        x1={x}
        y1={y1}
        x2={x}
...
export default function VectorField({direction, x, y}: { direction: number, x: number, y: number }) {
    const vectorField = [];
